            ...
            <div class="dropdown d-inline-block">
                <button type="button" class="btn btn-alt-secondary" id="page-header-notifications-dropdown" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    ...
                    @if (request()->user()->unreadNotifications()->count())
                        <span class="badge bg-warning rounded-pill">{{ request()->user()->unreadNotifications()->count() }}</span>
                    @endif
                </button>
                <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0" aria-labelledby="page-header-notifications-dropdown">
                    ...
                    <ul class="nav-items my-2">
                        @forelse (request()->user()->unreadNotifications()->limit(5)->orderBy('created_at', 'desc')->get() as $notification)
                            <li>
                                <a class="d-flex text-dark py-2">
                                    <div class="flex-shrink-0 mx-3">
                                        <i class="{{ \App\Modules\Notifications\Entities\Notification::getIcon($notification['data']['type']) }} fa-fw"></i>
                                    </div>
